// ECHO OS appkit — a minimal, dependency-free JSON reader.
//
// Just enough to pull fields out of the JSON the Spotify / Gmail / Google APIs
// return. Deliberately NOT a general-purpose library: read-only, no serialization,
// no exceptions. Malformed input yields a Null value rather than throwing — a
// backend that gets garbage back from the network degrades like any other failure.
//
// Why hand-rolled instead of a vendored library: the stub/CI build is
// zero-dependency by policy, and this parser (with its own tests) is small enough
// to audit at a glance. If a heavier dependency is ever justified, backends read
// through this same Json type and would not change.
#pragma once
