...
//
// LOCATION (Phase 22): this lived at memory/include/echo/memory/aes256.hpp until
// the caregiver link needed the same block cipher for its transport framing.
// Making companion-sync link echo::memory to reach it would have dragged SQLite
// and the whole on-device store into the module that talks OFF the device —
// precisely backwards. So the primitive moved down into echo::common (which every
// module already links) and echo/memory/aes256.hpp became an alias. The code is
// byte-for-byte the Phase 16 implementation; the KATs still guard it.
#pragma once
